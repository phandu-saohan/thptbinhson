...
export default function VanNgheBlock({ onNavigateHome }: { onNavigateHome?: () => void }) {
  ...
  const handleMarkDone = async (song: Song) => {
    const pwd = prompt('Nhập mật khẩu để chuyển bài hát sang bảng xếp hạng:');
    if (pwd === null) return;
    if (pwd !== '88888888') {
                            ...
                            {confirmDeleteId === song.id ? (
                              <div className="flex items-center gap-1">
                                ...
                                <button
                                  ...
                                  className="bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold text-[10px] px-2.5 py-1.5 rounded-full transition-all"
                                >
                                  Hủy
                                </button>
                              </div>
