...
export default function AvatarEditor({ frameSource = DEFAULT_FRAME }: { frameSource?: string }) {
  ...
  const shareImage = async () => {
    ...
    try {
      ...
      canvas.width = 1000;
      canvas.height = 1000;
      ctx.drawImage(userImg, croppedAreaPixels.x, croppedAreaPixels.y, croppedAreaPixels.width, croppedAreaPixels.height, 0, 0, 1000, 1000);
      ctx.drawImage(frameImg, 0, 0, 1000, 1000);

      const blob = await new Promise<Blob | null>((resolve) => canvas.toBlob(resolve, 'image/png'));
      if (!blob) return;

      const file = new File([blob], `Avatar_BinhSon_20Nam.png`, { type: 'image/png' });

      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'Avatar Kỷ Niệm 20 Năm THPT Bình Sơn',
          text: 'Tôi đã tạo xong khung avatar kỷ niệm 20 năm ngày ra trường. Cùng tham gia nhé!',
        });
      } else {
        const dataUrl = canvas.toDataURL('image/png');
        setResultImage(dataUrl);
      ...
      if (dataUrl) setResultImage(dataUrl);
    } finally {
      setIsGenerating(false);
    }
  };
        ...
        <div className="w-full flex flex-col items-center space-y-8">
          ...
          <div className="w-full max-w-[500px] bg-white p-4 md:p-6 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 space-y-5 md:space-y-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button
                onClick={shareImage}
                className="w-full flex items-center justify-center gap-2 py-4 bg-secondary text-on-secondary rounded-2xl shadow-md hover:opacity-90 transition active:scale-95 font-bold"
              >
                <span className="material-symbols-outlined">share</span>
                Chia sẻ lên MXH (FB, Zalo...)
              </button>
